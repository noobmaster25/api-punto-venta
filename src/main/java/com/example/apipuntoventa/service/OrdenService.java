...
public class OrdenService {
	...
	public OrdenDTO obtenerOrdenPorId(Integer id)  {
		Orden orden = ordenRepo.findById(id).orElseThrow(()->  new NotFoundException("orden con id "+id+" no existe"));
	...
	public OrdenDTO guardarOrden(OrdenNuevaDTO ordenNuevaDto)  {
		...
		if(cliente == null)throw new NotFoundException("cliente con id "+ordenNuevaDto.getClienteId()+" no existe");
		...
		for (DetalleOrdenNuevaDTO detalleOrdenNuevaDto : ordenNuevaDto.getDetallesOrdenNuevaDto()) {	
			...
			if(producto == null)throw new NotFoundException("producto con id "+detalleOrdenNuevaDto.getProductoId()+ " no existe");
	...
	public OrdenDTO actualizarOrden (Integer id , OrdenActualizadaDTO ordenActualizadaDto) {
		Orden orden = ordenRepo.findById(id).orElseThrow(()-> new NotFoundException("orden con id "+id+" no existe"));
		Cliente cliente = clienteRepo.findById(ordenActualizadaDto.getClienteId())
				.orElseThrow(()->new NotFoundException("cliente con id "+ordenActualizadaDto.getClienteId()+" no existe"));
	...
	private Orden mapearOrdenDeNuevaOrdenDTO(OrdenNuevaDTO ordenNuevaDto,Cliente cliente) {
		Orden ordenNueva = new Orden(ordenNuevaDto.getNumeroOrden(),
				ordenNuevaDto.getFechaCreacion(),
				cliente);
		
		return ordenNueva;
	}
	private Cliente obtenerClientePorId(Integer id) {
